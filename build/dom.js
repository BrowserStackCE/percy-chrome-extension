  ...
  (function (exports) {
        ...
        switch (elem.type) {
          ...
          case 'select-one':
            if (elem.selectedIndex !== -1) {
              cloneEl.options[elem.selectedIndex].setAttribute(
                'selected',
                'true'
              );
            }
    ...
    function serializeFrames(_ref) {
      ...
      for (let frame of dom.querySelectorAll('iframe')) {
        ...
        let cloneEl = clone.querySelector(
          `[data-percy-element-id="${percyElementId}"]`
        );
        let builtWithJs =
          !frame.srcdoc &&
          (!frame.src || frame.src.split(':')[0] === 'javascript');

        // delete frames within the head since they usually break pages when
        // rerendered and do not effect the visuals of a page
        if (
          (_clone$head = clone.head) !== null &&
          _clone$head !== void 0 &&
          _clone$head.contains(cloneEl)
        ) {
          cloneEl.remove();
          ...
          if (
            !builtWithJs &&
            !frame.contentWindow.performance.timing.loadEventEnd
          )
            continue;
    ...
    function markElement(domElement, disableShadowDOM) {
      ...
      if (
        [
          ...
          (_domElement$tagName = domElement.tagName) === null ||
            _domElement$tagName === void 0
            ? void 0
            : _domElement$tagName.toLowerCase()
        )
      ) {
    ...
    function styleSheetsMatch(sheetA, sheetB) {
      for (let i = 0; i < sheetA.cssRules.length; i++) {
        ...
        let ruleB =
          (_sheetB$cssRules$i = sheetB.cssRules[i]) === null ||
          _sheetB$cssRules$i === void 0
            ? void 0
            : _sheetB$cssRules$i.cssText;
        if (ruleA !== ruleB) return false;
      }
      return true;
    }
    function serializeCSSOM(_ref) {
      let { dom, clone, resources, cache } = _ref;
      // in-memory CSSOM into their respective DOM nodes.
      for (let styleSheet of dom.styleSheets) {
        if (isCSSOM(styleSheet)) {
          let styleId = styleSheet.ownerNode.getAttribute(
            'data-percy-element-id'
          );
          let cloneOwnerNode = clone.querySelector(
            `[data-percy-element-id="${styleId}"]`
          );
          if (styleSheetsMatch(styleSheet, styleSheetFromNode(cloneOwnerNode)))
            continue;
          ...
          style.innerHTML = Array.from(styleSheet.cssRules)
            ...
            .join('\n');
          cloneOwnerNode.parentNode.insertBefore(
            style,
            cloneOwnerNode.nextSibling
          );
      ...
      for (let sheet of dom.adoptedStyleSheets) {
        ...
        styleLink.setAttribute(
          'data-percy-adopted-stylesheets-serialized',
          'true'
        );
        styleLink.setAttribute(
          'data-percy-serialized-attribute-href',
          cache.get(sheet)
        );

        /* istanbul ignore next: tested, but coverage is stripped */
        if (
          clone.constructor.name === 'HTMLDocument' ||
          clone.constructor.name === 'DocumentFragment'
        ) {
    ...
    function serializeVideos(_ref) {
      let { dom, clone, resources, warnings } = _ref;
      for (let video of dom.querySelectorAll('video')) {
        ...
        cloneEl.setAttribute(
          'data-percy-serialized-attribute-poster',
          resource.url
        );
      }
    }
    ...
    function getOuterHTML(docElement) {
      ...
      return docElement.outerHTML.replace(
        '</html>',
        () => `${innerHTML}</html>`
      );
    }
    ...
    function serializeHTML(ctx) {
      ...
      html = html.replace(/ data-percy-serialized-attribute-(\w+?)=/gi, ' $1=');
    ...
    function serializeElements(ctx) {
      ...
      for (const shadowHost of ctx.dom.querySelectorAll(
        '[data-percy-shadow-host]'
      )) {
        let percyElementId = shadowHost.getAttribute('data-percy-element-id');
        let cloneShadowHost = ctx.clone.querySelector(
          `[data-percy-element-id="${percyElementId}"]`
        );
    ...
    function serializeDOM(options) {
      let {
        ...
        enableJavaScript = options === null || options === void 0
          ? void 0
          : options.enable_javascript,
        domTransformation = options === null || options === void 0
          ? void 0
          : options.dom_transformation,
        stringifyResponse = options === null || options === void 0
          ? void 0
          : options.stringify_response,
        disableShadowDOM = options === null || options === void 0
          ? void 0
      ...
      if (domTransformation) {
        try {
          ...
          if (typeof domTransformation === 'string')
            domTransformation = window.eval(domTransformation);
