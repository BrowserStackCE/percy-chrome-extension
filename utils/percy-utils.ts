...
import { PreferncesSchema, type Preferences } from "~schemas/preferences"
...
export class Percy {
  ...
  static getBuild() {
    return LocalStorage.get<PercyBuild>("build").then((build) => {
      return build
    })
  }
  ...
  static async isEnabled() {
    return fetch(`${baseurl}/percy/healthcheck`)
      .then((res) => {
        if (res.status == 200) {
          return res.json()
        } else {
...
*/
  ...
  static async stopPercy() {
    return fetch(`${baseurl}/percy/stop`)
      .then((res) => res.status == 200)
      .catch(() => false)
  }
  /*
Finalize -> getbuild from storage -> send build to desktop app
desktop app: 
-process request
- health check twice 
already running?
...
build url returned to extension: 
chrome extension: 
send percy build to desktop app

*/
  static async startPercy() {
    const preferences = await LocalStorage.get<Preferences>(
      "preferences",
    ...
    const discoveryOptions = {
      ...
      "request-headers":
        preferences.discoveryOptions?.["request-headers"]?.reduce(
          (map, item) => {
    ...
    const percyExtensionData = {
      build: build,
      preferences: preferences,
      discoveryOptions: discoveryOptions
    }

    console.log("Percy Extension Data: " + JSON.stringify(percyExtensionData))
    ...
    return fetch(`${appUrl}/percy/start`, {
      ...
      body: JSON.stringify({
        ...
        percyExtensionData: percyExtensionData
      })
    })
      .then((res) => {
